...
async function startServer() {
  ...
  app.post("/api/inquiry", (req, res) => {
    ...
    console.log("새로운 수강문의 접수:", { name, phone, courseInterest, preferredTime, hasNaeilCard, userCategory, message });
    return res.json({
      success: true,
      message: `${name}님의 수강 문의가 성공적으로 접수되었습니다. 빠르게 확인 후 안내 전화(033-433-1926)를 드리겠습니다.`,
  ...
  app.post("/api/ask-ai", async (req, res) => {
    try {
      ...
      const prompt = `
너는 '홍천 중앙정보처리학원(jahrd.com, 1969년 설립, 강원도 홍천군 홍천읍 신장대로 48 2층, 전화 033-433-1926)'의 친절하고 전문적인 AI 수강 상담 선생님이야.

[학원 주요 특징]
1. 1969년 설립된 50년 이상 전통의 홍천 대표 컴퓨터/IT 교육기관
